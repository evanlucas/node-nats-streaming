...
Stan.prototype.createConnection = function() {
  ...
  this.nc.on('connect', function() {
    ...
    that.nc.request(discoverSubject, new Buffer(req.serializeBinary()), {max:1}, function(msg) {
      var cr = proto.ConnectResponse.deserializeBinary(toByteArray(new Buffer(msg, 'binary')));
      if (cr.getError() !== "") {
        // connect failed
...
Stan.prototype.close = function() {
  ...
  this.nc.request(this.closeRequests, new Buffer(req.serializeBinary()), {max: 1}, function(msg) {
    ...
    nc.flush(function(){
      ...
      var cr = proto.CloseResponse.deserializeBinary(toByteArray(new Buffer(msg, 'binary')));
      var err = cr.getError();
      if(err && err.length > 0) {
...
Stan.prototype.processAck = function() {
  ...
  return function(msg) {
    //noinspection JSUnresolvedVariable
    var pa = proto.PubAck.deserializeBinary(toByteArray(new Buffer(msg, 'binary')));
    var guid = pa.getGuid();
...
Stan.prototype.subscribe = function(subject, qGroup, options) {
  ...
  var rsid = this.nc.request(this.subRequests, new Buffer(sr.serializeBinary()), {'max': 1}, function(msg) {
    //noinspection JSUnresolvedVariable
    var r = proto.SubscriptionResponse.deserializeBinary(toByteArray(new Buffer(msg, 'binary')));
    var err = r.getError();
    if (err && err.length !== 0) {
...
Stan.prototype.processMsg = function() {
  ...
  return function(rawMsg, reply, subject, sid) {
    ...
    try {
      //noinspection JSUnresolvedVariable
      var m = proto.MsgProto.deserializeBinary(toByteArray(new Buffer(rawMsg, 'binary')));
